...
(function ($) {
    ...
    FileInput.prototype = {
        ...
        init: function (options) {
            ...
            self.$SuccessContainer = getElement(options, 'elErrorContainer', self.$previewContainer.find('.kv-fileinput-upload'));
                ...
                setTimeout(function () {
                    if (data.error === undefined) {
                        ...
                        if (data.message != null) {
                            ...
                            setIndicator('indicatorSuccess', 'indicatorSuccessTitle');
                            self.showUploadError(data.message);
                            $btnUpload.hide();
                            $btnDelete.hide();
                            ...
                            self.showUploadError(data.error);
                        }
        ...
        showUploadSuccess: function (msg, params, event) {
            var self = this, $error = self.$SuccessContainer, ev = event || 'fileuploaded';
            if ($error.find('ul').length === 0) {
                //$error.html('<ul><li>' + msg + '</li></ul>');
    ...
    $.fn.fileinput.locales.en = {
        ...
        msgSizeTooLarge: 'El archivo "{name}" (<b>{size} KB</b>) a excedido el limite maximo permitido <b>{maxSize} KB</b>. Revise por favor',
        msgFilesTooLess: 'You must select at least <b>{n}</b> {files} to upload. Please retry your upload!',
        msgFilesTooMany: 'Number of files selected for upload <b>({n})</b> exceeds maximum allowed limit of <b>{m}</b>. Please retry your upload!',
        msgFileNotFound: 'Archivo "{name}" no cargado!',
        msgFileSecured: 'Security restrictions prevent reading the file "{name}".',
        msgFileNotReadable: 'File "{name}" is not readable.',
        msgFilePreviewAborted: 'File preview aborted for "{name}".',
        msgFilePreviewError: 'An error occurred while reading the file "{name}".',
        msgInvalidFileType: 'Invalid type for file "{name}". Only "{types}" files are supported.',
        msgInvalidFileExtension: 'La extension del archivo "{name}" no es complatible, revise que sea con extension "{extensions}".',
        msgValidationError: 'Error en el Archivo',
        ...
        dropZoneTitle: 'Arrastre los Archivos a cargar'
    };
